...
sprintImage.save(path.join(targetImagesDirPath, '../dist/', 'sprite.png'));

// 生成css文件
let content = '';
for (let i = 0, len = regions.length; i < len; i++) {
  const name = regions[i].path.split('/')[1].split('.')[0];
  console.log(regions[i]);
  let _imgCss = `&-${name} { @include iconmapBaseSet(-${regions[i].x}rpx, -${regions[i].y}rpx, ${regions[i].width}rpx, ${regions[i].height}rpx); }\n`;
  content = content + _imgCss;
}
fs.writeFileSync(path.join(targetImagesDirPath, '../dist/', './sprite-css.txt'), content);
